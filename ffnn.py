...
def convert_to_index_sequences(data, word_to_index, has_labels=True):
    """
    Vectorize data with or without labels.
    """
    vectorized_data = []
    if has_labels:
        ...
        for document in data:
            index_sequence = [word_to_index.get(word, word_to_index[unk]) for word in document]
            vectorized_data.append(index_sequence)
...
def load_data(data, has_labels=True):
    ...
    for elt in unsplit_data:
        if has_labels:
            ...
            data_array.append(elt["text"].split())
    return data_array
...
if __name__ == "__main__":
    ...
    parser.add_argument("--predict_only", type=bool, required=False, help="Use tag if train data doesn't have true labels")
    ...
    if args.test_data is not None:
        has_labels = args.predict_only is None
        test_data = load_data(args.test_data, has_labels)
    else:
        test_data = None
    ...
    test_data = convert_to_index_sequences(test_data, word_to_index, has_labels)
    ...
    if (args.test_data is not None) and (args.predict_only is None):
        print("========== Testing ==========")
        correct = 0
        total = 0
        all_preds = []
    ...
    # ======= Predict and return labels only (no evaluation) =======
    if (args.test_data is not None) and (args.predict_only is not None):
        print("========== Predicting Labels Only ==========")
        predicted_labels = []

        model.eval()
        with torch.no_grad():
            for input_indices in test_data:
                input_tensor = torch.tensor(input_indices, dtype=torch.long)
                logits = model(input_tensor)

                # Convert logits to predicted class using CORAL thresholds
                predicted_label = int(torch.sum(torch.sigmoid(logits) > 0.5).item())
                predicted_label = max(0, min(4, predicted_label))  # Clamp between 0–4
                predicted_labels.append(predicted_label)

        print("Predicted labels (0–4):")
